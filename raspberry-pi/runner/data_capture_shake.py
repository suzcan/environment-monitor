...
while True:
    ...
    print("Waiting for data is: " + str(waiting_for_data))
    ...
    if packet is None:
        ...
        if(waiting_for_data):
            waited = time.time() - start_data_wait
            print("Waited is: " + str(waited))
            print("Timeout is: " + str(timeout))
            if(waited > timeout):
                print(str(getTime()) + " LOG: timed out while waiting for correct data to arrive")
                waiting_for_data = False
            if(packet[2] == prev_header):
                try:
                    data = re.findall(pattern, packet_text)
                    print(data)

                    env_data = {
                            'time': math.floor(getTime().timestamp()), #data[0],
                            'devID': data[1],
                            'ana0': data[2],
                            'ana1': data[3],
                            'ana2': data[4],
                    ...
                    db.close()
                    msg = bytes("DN", "utf-8");
                    rfm9x.send(msg, tx_header=(prev_header, 255, receiver_id, 0))
                    waiting_for_data = False
            ...
            if("RDY" in packet_text):
                print(str(getTime()) + " LOG: recieved RDY from" + str(packet[2]))
                prev_header = packet[2]
                msg = bytes("ACK", "utf-8")
                rfm9x.send(msg, tx_header=(prev_header, 255, receiver_id, 0))
